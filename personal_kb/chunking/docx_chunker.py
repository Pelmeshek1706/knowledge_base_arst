...
class DocxChunker(BaseChunker):
    ...
    def __init__(self, *, chunk_size: int = 1600, chunk_overlap: int = 200) -> None:
        if chunk_size <= 0:
    ...
    def chunk(self, document: ParsedDocument, *, document_id: str) -> list[ChunkRecord]:
        ...
        for block in document.structured_blocks:
            ...
            source_ref = self._as_source_ref(document, block, heading_path)
            section_text = self._compose_section_text(heading_path, text)
            ...
            for chunk_text in self._split_section_text(heading_path, text):
                chunks.append(
                    self._make_chunk(
                        document_id=document_id,
                        chunk_index=len(chunks),
    ...
    def _compose_section_text(self, heading_path: list[str], body_text: str) -> str:
        if not heading_path:
            return body_text
        heading_context = " > ".join(heading_path)
        return f"{heading_context}\n\n{body_text}"

    def _split_section_text(self, heading_path: list[str], body_text: str) -> list[str]:
        ...
        while start < len(body_text):
            end = min(len(body_text), start + body_chunk_size)
            chunks.append(f"{prefix}{body_text[start:end]}")
    ...
    def _as_heading_path(self, block: dict[str, object]) -> list[str]:
    ...
    def _as_source_ref(
        self, document: ParsedDocument, block: dict[str, object], heading_path: list[str]
    ) -> SourceRef:
        raw_ref = block.get("source_ref")
        if isinstance(raw_ref, dict):
            section = str(raw_ref.get("section", "") or "").strip() or None
            if section is None and heading_path:
